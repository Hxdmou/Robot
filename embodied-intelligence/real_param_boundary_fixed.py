...
generate_boundary_report(
    report_filename=os.path.join(PROJECT_ROOT, "boundary_fixed_report.txt"),
    log_filename=os.path.join(PROJECT_ROOT, "boundary_fixed_log.csv"),
...
print("[BOUNDARY] 报告已生成: {output_path}/boundary_fixed_report.txt")
...
try:
    ...
    while True:
        ...
        if _loop_count > _MAX_LOOPS:
            print("[BOUNDARY] 运行超过循环上限，强制退出。")
            break
        if time.time() - _start_time > _MAX_SECONDS:
            print("[BOUNDARY] 运行超时（24小时），强制退出。")
            break
        time.sleep(1)
