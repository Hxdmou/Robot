...
FOOTER_TEXT = '具身智能&AI产业最新进展 · 2026年8月22日 · 商务汇报'
...
def _content_page_render(prs, part_num, title, items, key_suffix, page_label):
    ...
    # V3.39：文本块按实际内容高度垂直居中（无卡片背景），杜绝大段距空隙
    content_h_in = content_h_pt / 72.0
    box_h = min(CONTENT_H - PAD_TOP - PAD_BOT, content_h_in)
...
def _detail_page_render(prs, part_num, title, detail_title, d_items, key_suffix, page_label):
    ...
    # V3.39：文本块按实际内容高度垂直居中（无竖条装饰），杜绝大段距空隙
    content_h_in = content_h_pt / 72.0
    box_h = min(region_h - d_PAD_TOP - d_PAD_BOT, content_h_in)
...
def cover_page(prs):
    ...
    tb(slide, block_start_x + btn_w + btn_gap, 6.0, date_w, 0.45, '2026年8月22日', sz=16, b=True, c=WHITE, al=PP_ALIGN.CENTER, an=MSO_ANCHOR.MIDDLE)
    # 底部装饰线
    rect(slide, 0, SLIDE_H - 0.36, SLIDE_W, 0.04, fc=GOLD)
    tb(slide, 0, SLIDE_H - 0.28, SLIDE_W, 0.2, FOOTER_TEXT, sz=8, c=MGRAY, al=PP_ALIGN.CENTER, an=MSO_ANCHOR.MIDDLE)
