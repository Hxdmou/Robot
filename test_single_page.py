# -*- coding: utf-8 -*-
"""
单页测试脚本 - 只测试最长的消费电子20条细节内容，加红色边界线
用来验证参数是否正确，不生成完整47页，节省时间
"""
...
def tb(slide, x, y, w, h, text, sz=12, b=False, c=None, al=PP_ALIGN.LEFT, an=MSO_ANCHOR.TOP):
...
def add_page_header(slide, part_num, title, margin, content_w, header_y):
    rect(slide, 0, 0, SLIDE_W, 0.06, fc=GOLD)
    rrect(slide, 0.2, 0.16, 0.9, 0.36, fc=GOLD)
    tb(slide, 0.2, 0.16, 0.9, 0.36, part_num, sz=9, b=True, c=DARK_BLUE, al=PP_ALIGN.CENTER, an=MSO_ANCHOR.MIDDLE)
    tb(slide, 1.25, 0.14, 10, 0.4, title, sz=15, b=True, c=WHITE, an=MSO_ANCHOR.MIDDLE)
    rect(slide, margin, header_y, content_w, 0.02, fc=RGBColor(0x20, 0x35, 0x60))

def add_page_tag(slide, tag_text, tag_color):
    rrect(slide, SLIDE_W - 1.6, 0.18, 1.3, 0.32, fc=tag_color)
    tb(slide, SLIDE_W - 1.6, 0.2, 1.3, 0.28, tag_text, sz=9, b=True, c=WHITE, al=PP_ALIGN.CENTER)

# 消费电子最长的20条细节内容（真实完整数据）
detail_items_long = [
...
# 测试不同参数组合
test_configs = [
    # (margin, content_top, content_bottom, space_after, label)
    (0.12, 0.66, 7.38, 0, 'cfg1: m=0.12, t=0.66, b=7.38, sa=0'),
]

for cfg_idx, (margin, content_top, content_bottom, space_after, label) in enumerate(test_configs):
    prs = Presentation()
    ...
    # 页眉
    add_page_header(slide, 'PART 12', '消费电子：AI终端普及（内容细节测试页）', margin, content_w, content_top - 0.02)
    add_page_tag(slide, '细节描述（测试）', GOLD)
    
    # 画卡片边界（红色）
    card = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(margin), Inches(content_top), Inches(content_w), Inches(content_h))
    card.fill.solid()
    card.fill.fore_color.rgb = MID_BLUE
    card.line.color.rgb = RED
    card.line.width = Pt(2)
    # 左侧金色装饰条
    rect(slide, margin, content_top, 0.06, content_h, fc=GOLD)
    
    # 文本框：直接拉满卡片
    text_x = margin + 0.07
    text_y = content_top + 0.04
    text_w = content_w - 0.12
    text_h = content_h - 0.08
    
    # 画文本框边界（蓝色）
    tb_border = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(text_x), Inches(text_y), Inches(text_w), Inches(text_h))
    tb_border.fill.background()
    tb_border.line.color.rgb = RGBColor(0x00, 0xFF, 0xFF)
    tb_border.line.width = Pt(1)
    ...
    # 添加20条内容
    add_bullets(tf, detail_items_long, start_idx=1, sz=10, line_spacing=1.1, space_after=space_after)
    
    # 页脚
    tb(slide, 0, content_bottom + 0.02, SLIDE_W, 0.1, f'测试配置：{label}', sz=7, c=LGRAY, al=PP_ALIGN.CENTER)
    
    out_path = r'F:\个人作品\具身智能\test_single_page.pptx'
    prs.save(out_path)
    print(f'单页测试PPT已生成：{out_path}')
    print(f'  配置：{label}')
    print(f'  卡片高度：{content_h:.2f}英寸')
    print(f'  文本框高度：{text_h:.2f}英寸')
    print('  红色框：卡片边界，蓝色框：文本框边界')
    print('  请打开查看是否溢出或有空隙')
